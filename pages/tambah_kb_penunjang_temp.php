<?php
	...
	if (isset($_POST['simpan'])) {
		...
		if (in_array($ekstensi, $ekstensi_diperbolehkan) === true && in_array($ekstensi1, $ekstensi_diperbolehkan1) === true) {
			$query = "INSERT INTO kb_penunjang_lain_temp (nomor_kbpn, jenis_kegiatan, bk_buktipenugasan, bk_sks, masa_penugasan, kinerja_sks, rekomendasi, kb_dokumen, id_user) 
					VALUES ('$id_kbpn', '$jenis_kegiatan', '$nama1', '$bk_sks', '$masa_penugasan', '$kinerja_sks', '$rekomendasi', '$nama', '$id_user')";

//			var_dump( $query);
			$result = mysqli_query($conn, $query) or die(mysqli_error($conn));

			if ($result) {
				move_uploaded_file($file_tmp, '../files/kb_penunjang_lain/'.$nama);
				move_uploaded_file($file_tmp1, '../files/kb_penunjang_lain/'.$nama1);
				echo '<script> alert("Data Berhasil Ditambah")
				window.location = "../tambah_kb_penunjang_lain.php"
				</script>';
			}
			else {

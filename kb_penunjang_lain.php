                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    <tbody>
									<?php
									$query  = "SELECT * FROM kb_penunjang_lain WHERE id_user = '$id_user' AND tahun_ajaran = (SELECT tahun FROM tahun_ajaran WHERE status = 1)";
									$result = mysqli_query( $conn, $query );
									while ( $row = mysqli_fetch_array( $result ) ) {
										?>
                                        <tr>
                                            <td><?php echo $row['nomor_kbpn'] ?></td>
                                                ...
                                                <div class="btn-toolbar" role="toolbar"
                                                    ...
                                                    <div class="btn-group" role="group" aria-label="First group">
                                                        <button type="button" class="btn btn-warning waves-effect"
                                                                onclick="window.location.href='ubah_kb_penunjang_lain.php?id_kbpn=<?php echo $row['id_kbpn']; ?>'">
                                                            Ubah
                                                        </button>
                                                        <button type="button" class="btn btn-danger waves-effect"
                                                                onclick="window.location.href='pages/hapus_kb_penunjang_lain.php?id=<?php echo $row['id_kbpn']; ?>'">
                                                            Hapus
                                                        </button>
                                                    </div>
                                                </div>
